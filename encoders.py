...
class GraphConv(nn.Module):
    def __init__(self, input_dim, output_dim, add_self=False, normalize_embedding=False,
            dropout=0.0, bias=True, cpu=False):
    ...
    def forward(self, x, adj):
        ...
        # print("GraphConv forward: x shape {}\t adj shape {}".format(x.size(), adj.size()))
        y = torch.matmul(adj, x)
        # print("GraphConv forward: y shape {}".format(y.size()))
        if self.add_self:
            y += x
...
class GcnEncoderGraph(nn.Module):
    ...
    def build_conv_layers(self, input_dim, hidden_dim, embedding_dim, num_layers, add_self,
        ...
        conv_first = GraphConv(input_dim=input_dim, output_dim=hidden_dim, add_self=add_self,
                normalize_embedding=normalize, bias=self.bias, cpu=self.cpu)
        conv_block = nn.ModuleList(
                [GraphConv(input_dim=hidden_dim, output_dim=hidden_dim, add_self=add_self,
                        normalize_embedding=normalize, dropout=dropout, bias=self.bias, cpu=self.cpu) 
    ...
    def gcn_forward(self, x, adj, conv_first, conv_block, conv_last, embedding_mask=None):
        ...
        # print("GcnEncoderGraph gcn_forward : x shape {}\t adj shape {}".format(x.size(), adj.size()))
        x = conv_first(x, adj)
        # print("GcnEncoderGraph gcn_forward : conv_first output shape {}".format(x.size()))
        x = self.act(x)
        if self.bn:
            x = self.apply_bn(x)
        x_all = [x]
    ...
    def loss(self, pred, label, typee='softmax'):
        # softmax + CE
        if typee == 'softmax':
            return F.cross_entropy(pred, label, reduction='mean')
            # return F.nll_loss(F.log_softmax(pred, dim=1), label)
            # print("pred shape:{} dtype:{},\t label shape:{} dtype:{}\t after converting dtype:{}".format(pred.shape, pred.dtype, label.shape, label.dtype, label.float().dtype))
            # return F.kl_div(pred, F.one_hot(label,pred.shape[1]).float(), reduction='batchmean')
            # return F.hinge_embedding_loss(pred, F.one_hot(label, pred.shape[1]).float())
...
class GcnSet2SetEncoder(GcnEncoderGraph):
    def __init__(self, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        self.s2s = Set2Set(self.pred_input_dim, self.pred_input_dim * 2, args.cpu)
...
class SoftPoolingGcnEncoder_my_module_base(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
            assign_hidden_dim, assign_ratio=0.25, assign_num_layers=-1, num_pooling=1,
            pred_hidden_dims=[50], concat=True, bn=True, dropout=0.0, linkpred=True,
            assign_input_dim=-1, args=None):
        ...
        super(SoftPoolingGcnEncoder_my_module_base, self).__init__(input_dim, hidden_dim, embedding_dim, label_dim,
                num_layers, pred_hidden_dims=pred_hidden_dims, concat=concat, args=args)
        add_self = not concat
        self.num_pooling = num_pooling
        self.linkpred = linkpred
    ...
    def forward(self, x, adj, batch_num_nodes, **kwargs):
        ...
        #    self.assign_tensor = self.assign_tensor * embedding_mask
        # [batch_size x num_nodes x embedding_dim]
        # print("in base : x shape {}\t adj shape {}".format(x.size(), adj.size()))
        embedding_tensor = self.gcn_forward(x, adj,
                self.conv_first, self.conv_block, self.conv_last, embedding_mask)

        # print("in base forward: embedding_tensor shape: {}".format(embedding_tensor.size()))
        out, _ = torch.max(embedding_tensor, dim=1)
        out_all.append(out)
        if self.num_aggs == 2:
            out = torch.sum(embedding_tensor, dim=1)
    ...
    def loss(self, pred, label):
        # print("base model: loss processing")
        return super(SoftPoolingGcnEncoder_my_module_base, self).loss(pred, label)

class SoftPoolingGcnEncoder_4_ast_n_pdg(SoftPoolingGcnEncoder_my_module_base):
    def __init__(self, max_num_nodes_dict, input_dim_dict, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        key = 'ast'
        max_num_nodes = max_num_nodes_dict[key]
        input_dim = input_dim_dict[key]
        assign_input_dim = assign_input_dim_dict[key]
        super(SoftPoolingGcnEncoder_4_ast_n_pdg, self).__init__(
        ...
        self.ast_module = SoftPoolingGcnEncoder_my_module_base(
            max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
            assign_hidden_dim, assign_ratio=assign_ratio, num_pooling=num_pooling, bn=bn, dropout=dropout,
            linkpred=linkpred, assign_input_dim=assign_input_dim, args=args)
        ...
        input_dim = input_dim_dict[key]
        assign_input_dim = assign_input_dim_dict[key]
        self.pdg_module = SoftPoolingGcnEncoder_my_module_base(
            max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
            assign_hidden_dim, assign_ratio=assign_ratio, num_pooling=num_pooling, bn=bn, dropout=dropout,
            linkpred=linkpred, assign_input_dim=assign_input_dim, args=args)
    ...
    def forward(self, x_ast, adj_ast, batch_num_nodes_ast, x_pdg, adj_pdg, batch_num_nodes_pdg, **kwargs):
        ...
        # trying concatenation of ast and pdg
        # pre_input = torch.cat((ast_output, pdg_output), dim=1)
        # trying sum of ast and pdg
        pre_input = torch.add(ast_output, pdg_output)
        # print("shape of pre_input: {}".format(pre_input.shape))
        # trying average of pdg and ast
        # combin_ast_pdg = torch.stack([ast_output, pdg_output])
        # print("combin_ast_pdg shape {}".format(combin_ast_pdg.shape))
        # pre_input = torch.mean(combin_ast_pdg, 0)
        # trying weighted sum
        # w_ast = 0.35
        # w_pdg = 0.65
        # pre_input = torch.add(torch.mul(ast_output, w_ast), torch.mul(pdg_output, w_pdg))

        ypred = self.pred_model(pre_input)
        return ypred
    ...
    def loss(self, pred, label, adj=None, batch_num_nodes=None, adj_hop=1):
        ...
        '''
        eps = 1e-7
        loss = super(SoftPoolingGcnEncoder_4_ast_n_pdg, self).loss(pred, label)
        # print("my model loss: loss {}".format(loss))
        # print("my model loss: linkpred {}".format(self.linkpred))
        if self.linkpred:
            max_num_nodes = adj.size()[1]
            pred_adj0 = self.assign_tensor @ torch.transpose(self.assign_tensor, 1, 2) 
            tmp = pred_adj0
